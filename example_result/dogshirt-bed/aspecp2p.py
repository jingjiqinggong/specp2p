...
set_gl()
from p2p.p2p import *
image_path1 =  "./tedbench/originals/two_dogs_with_checkered_shirts1.jpg"
image_path2 = "./gradio_app/bed3.jpg"  # 这里最好用原图吧
image_cut = "./gradio_app/bed3.jpg"        # imga2 用AI抠图 去掉背景的  为了获取mask  https://photokit.com/editor/?lang=zh https://tool.lu/cutout/
...
prompts = ["two dogs standing on a board, lake behind",
           "",
           "two dogs standing on a mattress, in the bedroom",  # 或者bed
]
target_words = ["mattress","bedroom"]          # 记得设置成和和local blend的相同哦
...
gl.speckv_step = (8,50)  # 优先级比 self_replace_steps 高
...
if hasattr(gl,'speckvs'):
    editor_half = AttHalfMutualMask(thres=0.2, mask_s=mask_image[0,0,:,:], 
...
blend_word = None
blend_word = (("board","lake",), ("mattress","bedroom",)) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
substruct_words = None
# substruct_words = (("dogs",),("dogs",))
eq_params = None
# eq_params = {"words": ("vase",), "values": (2,)} 
