...
image_path2 = "./gradio_app/shirt.jpg"  # 这里最好用原图吧
image_cut = "./gradio_app/shirt.png"        # imga2 用AI抠图 去掉背景的  为了获取mask  https://photokit.com/editor/?lang=zh https://tool.lu/cutout/
...
prompts = ["two dogs wearing red clothes",
           "",
           "two dogs wearing blue clothes",
]
target_words = ["blue"]        # 记得设置成和和local blend的相同哦
...
gl.spec_mask_type = 2   # 0: 两个mask都用   1: 只用mask_s   2: 只用mask_t   其他值: 都不用mask

# thres 老虎 replace 0.5  refine 0.3
if hasattr(gl,'speckvs'):
    editor_half = AttHalfMutualMask(thres=0.3, mask_s=mask_image[0,0,:,:], 
...
self_replace_steps = 0.8 # 哈士奇的图 用0.6吧
blend_word = None
blend_word = (("red","clothes"), ("blue",)) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
