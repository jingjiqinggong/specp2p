...
image_path2 = "./gradio_app/myvase.jpg"  # 这里最好用原图吧
image_cut = "./gradio_app/myvase.png"      # imga2 用AI抠图 去掉背景的  为了获取mask  https://photokit.com/editor/?lang=zh https://tool.lu/cutout/
...
target_words = ["vase"]        # 记得设置成和和local blend的相同哦
...
if hasattr(gl,'speckvs'):
    editor_half = AttHalfMutualMask(thres=0.45, mask_s=mask_image[0,0,:,:], 
...
self_replace_steps = 0.4 # 哈士奇的图 用0.6吧
blend_word = None
blend_word = (("vase",), ("vase",))# for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
