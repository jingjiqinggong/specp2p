...
set_gl()
from p2p.p2p import *
image_path1 =  "./gradio_app/toy_rabbit3.jpg"
image_path2 = "./gradio_app/cake33.jpg"  # 这里最好用原图吧
image_cut = "./gradio_app/cake33.jpg"    # imga2 用AI抠图 去掉背景的  为了获取mask  https://photokit.com/editor/?lang=zh https://tool.lu/cutout/
...
prompts = ["A human-like toy is sitting on a cushion",
           "",
           "A human-like toy is sitting on a birthday cake",
]
target_words = ["cake"]       # 记得设置成和和local blend的相同哦
...
blend_word = None
blend_word = (("cushion",), ("cake",)) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
...
controller = make_controller(prompts, False, cross_replace_steps, self_replace_steps, blend_word, eq_params,
                             substruct_words,
                             th=(0.3,0.45)) # False:Refinement   th0: words的  th1: substruct word 的
